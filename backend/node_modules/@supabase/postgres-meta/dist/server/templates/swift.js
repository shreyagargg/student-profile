function formatForSwiftSchemaName(schema) {
    return `${formatForSwiftTypeName(schema)}Schema`;
}
function pgEnumToSwiftEnum(pgEnum) {
    return {
...
function pgTypeToSwiftStruct(table, columns, operation, { types, views, tables, }) {
...
function generateCodingKeysEnumFromAttributes(attributes) {
...
function pgCompositeTypeToSwiftStruct(type, { types, views, tables, }) {
    ...
    const attributeEntries = typeWithRetrievedAttributes.attributes.map((attribute) => {
        return {
...
function generateProtocolConformances(protocols) {
    return protocols.length === 0 ? '' : `: ${protocols.join(', ')}`;
}
function generateEnum(enum_, { accessControl, level }) {
    return [
        `${ident(level)}${accessControl} enum ${enum_.formattedEnumName}${generateProtocolConformances(enum_.protocolConformances)} {`,
        ...enum_.cases.map((case_) => `${ident(level + 1)}case ${case_.formattedName} = "${case_.rawValue}"`),
        `${ident(level)}}`,
    ];
}
function generateStruct(struct, { accessControl, level }) {
    ...
    if (identity && identity.formattedAttributeName !== 'id') {
        output.push(`${ident(level + 1)}${accessControl} var id: ${identity.formattedType} { ${identity.formattedAttributeName} }`);
    }
    output.push(...struct.attributes.map((attribute) => `${ident(level + 1)}${accessControl} let ${attribute.formattedAttributeName}: ${attribute.formattedType}`));
...
export const apply = async ({ schemas, tables, foreignTables, views, materializedViews, columns, types, accessControl, }) => {
    ...
    let output = [
        ...
        ...schemas
            ...
            return [
                ...
                ...schemaEnums.flatMap((enum_) => generateEnum(pgEnumToSwiftEnum(enum_), { accessControl, level: 1 })),
                ...schemaTables.flatMap((table) => ['Select', 'Insert', 'Update']
                    .map((operation) => pgTypeToSwiftStruct(table, columnsByTableId[table.id], operation, {
                    types,
                    views,
                    tables,
                }))
                    .flatMap((struct) => generateStruct(struct, { accessControl, level: 1 }))),
                ...schemaViews.flatMap((view) => generateStruct(pgTypeToSwiftStruct(view, columnsByTableId[view.id], 'Select', {
                    types,
                    views,
                    tables,
                }), { accessControl, level: 1 })),
                ...schemaCompositeTypes.flatMap((type) => generateStruct(pgCompositeTypeToSwiftStruct(type, { types, views, tables }), {
                    accessControl,
                    level: 1,
                })),
                '}',
...
const pgTypeToSwiftType = (pgType, nullable, { types, views, tables, }) => {
    let swiftType;
    if (pgType === 'bool') {
        swiftType = 'Bool';
    }
...
function ident(level, options = { width: 2 }) {
 ...
 * ```
 */
function formatForSwiftTypeName(name) {
 ...
 * ```
 */
function formatForSwiftPropertyName(name) {
