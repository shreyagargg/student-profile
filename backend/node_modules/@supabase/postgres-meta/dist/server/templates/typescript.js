...
import { GENERATE_TYPES_DEFAULT_SCHEMA, VALID_FUNCTION_ARGS_MODE, VALID_UNNAMED_FUNCTION_ARG_TYPES, } from '../constants.js';
export const apply = async ({ schemas, tables, foreignTables, views, materializedViews, columns, relationships, functions, types, detectOneToOneRelationships, postgrestVersion, }) => {
    schemas.sort((a, b) => a.name.localeCompare(b.name));
    relationships.sort((a, b) => a.foreign_key_name.localeCompare(b.foreign_key_name) ||
    ...
    const introspectionBySchema = Object.fromEntries(schemas.map((s) => [
        s.name,
    ...
    function getRelationships(object, relationships) {
        return relationships.filter((relationship) => relationship.schema === object.schema &&
    ...
    const getFunctionReturnType = (schema, fn) => {
        ...
        const relation = introspectionBySchema[schema.name]?.tables.find(({ table: { id } }) => id === fn.return_type_relation_id)?.table ||
            introspectionBySchema[schema.name]?.views.find(({ view: { id } }) => id === fn.return_type_relation_id)?.view;
        if (relation) {
            return `{
              ${columnsByTableId[relation.id]
                .map((column) => generateColumnTsDefinition(schema, {
                name: column.name,
  ...
  ${schemas.map((schema) => {
        ...
        return `${JSON.stringify(schema.name)}: {
          Tables: {
            ...
            ? '[_ in never]: never'
            : schemaTables.map(({ table, relationships }) => `${JSON.stringify(table.name)}: {
                  Row: {
                    ${[
          ...
          Views: {
            ...
            ? '[_ in never]: never'
            : schemaViews.map(({ view, relationships }) => `${JSON.stringify(view.name)}: {
                  Row: {
                    ${[
                ...
                ...schemaFunctions
                    ...
                    .map(({ fn }) => `${JSON.stringify(fn.name)}: ${generateNullableUnionTsType(getFunctionReturnType(schema, fn), true)}`),
            ]}
                  }
                  ${view.is_updatable
                ? `Insert: {
          ...
          Functions: {
            ...
            for (const fnName in schemaFunctionsGroupedByName) {
                schemaFunctionsGroupedByName[fnName].sort((a, b) => b.fn.definition.localeCompare(a.fn.definition));
            }
          ...
          CompositeTypes: {
            ...
            : schemaCompositeTypes.map(({ name, attributes }) => `${JSON.stringify(name)}: {
                ...
                let tsType = 'unknown';
                if (type) {
                    tsType = `${generateNullableUnionTsType(pgTypeToTsType(schema, type.name, {
                        types,
                        schemas,
                        tables,
                        views,
    ...
    if (pgType === 'bool') {
        ...
        if (tableRowTypes.length > 0) {
            ...
            if (schemas.some(({ name }) => name === tableRowType.schema)) {
                return `Database[${JSON.stringify(tableRowType.schema)}]['Tables'][${JSON.stringify(tableRowType.name)}]['Row']`;
            }
            return 'unknown';
        }
        ...
        if (viewRowTypes.length > 0) {
            ...
            if (schemas.some(({ name }) => name === viewRowType.schema)) {
                return `Database[${JSON.stringify(viewRowType.schema)}]['Views'][${JSON.stringify(viewRowType.name)}]['Row']`;
            }
            return 'unknown';
        }
        return 'unknown';
