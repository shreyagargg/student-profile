import { ALL_TYPES } from './schema/index.js';
...
export class TriplitError extends Error {
    ...
    constructor(contextMessage, ...args) {
        super(...args);
    ...
    static fromJson(json) {
        ...
        if (json.baseMessage)
            error.baseMessage = json.baseMessage;
        if (json.name)
...
export class EntityNotFoundError extends TriplitError {
    constructor(entityId, collectionName, ...args) {
        super(...args);
...
export class InvalidCollectionNameError extends TriplitError {
    constructor(collectionName, ...args) {
        super(...args);
...
export class WritePermissionError extends TriplitError {
    constructor(collection, entityId, operation, sessionRoles, ...args) {
        super(...args);
        this.name = 'WritePermissionError';
        this.baseMessage = `'${operation}' permission for the collection '${collection}' prevented the ${operation === 'insert'
            ? 'insertion'
            : operation === 'delete'
                ? 'deletion'
                : 'update'} of the entity with id '${entityId}'. The provided session roles were [${sessionRoles
            .map((m) => m.key)
            .join(', ')}].`;
...
export class DBSerializationError extends TriplitError {
    constructor(targetType, erroneousValue, ...args) {
        super(...args);
...
export class JSONSerializationError extends TriplitError {
    constructor(erroneousValue, expectedType, ...args) {
        super(...args);
...
export class SessionVariableNotFoundError extends TriplitError {
    constructor(variableName, scope, scopeVars, ...args) {
        super(...args);
        this.name = 'SessionVariableNotFoundError';
        this.baseMessage = `\'${variableName}\' could not be found in the variables for this query. The available ${scope} variables are: ${Object.keys(scopeVars ?? {})
            .map((v) => `\'${scope}.${v}\'`)
            .join(', ')}`;
        this.status = STATUS_CODES['Bad Request'];
    }
...
export class RelationDoesNotExistError extends TriplitError {
    constructor(relationName, alias, collectionName, ...args) {
        super(...args);
...
export class QueryClauseFormattingError extends TriplitError {
    constructor(clauseType, clause, ...args) {
        super(...args);
