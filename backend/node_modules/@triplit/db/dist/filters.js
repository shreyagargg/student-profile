...
export async function satisfiesFilters(entity, filters, queryEngine) {
    ...
    for (const idx of priorityOrder) {
        ...
        if (!isSatisfied)
            break; // Short-circuit if any filter fails
    }
    return isSatisfied;
}
export async function satisfiesFilter(entity, filter, queryEngine) {
...
export function satisfiesNonRelationalFilter(entity, filter, ignoreSubQueries = false) {
    if (isBooleanFilter(filter))
        return filter;
    if (isFilterGroup(filter)) {
        const { mod, filters } = filter;
        if (mod === 'and') {
            return filters.every((f) => satisfiesNonRelationalFilter(entity, f, ignoreSubQueries));
        }
        if (mod === 'or') {
            return filters.some((f) => satisfiesNonRelationalFilter(entity, f, ignoreSubQueries));
        }
        return false;
    }
    if (isSubQueryFilter(filter)) {
        ...
        throw new Error(`Subquery filters should be filtered out before this point, found ${filter}`);
    }
    return satisfiesFilterStatement(entity, filter);
}
function satisfiesFilterStatement(entity, filter) {
...
function evaluateFilterStatement(value, op, filterValue) {
    /**
    ...
    if (op.startsWith(SET_OP_PREFIX)) {
        ...
        if (typeof value !== 'object' && value !== null && value !== undefined)
            throw new InvalidFilterError(`The operator requires a set value, but got ${value}`);
    ...
    switch (op) {
        case '=':
            ...
            if (hasNoValue(value) && hasNoValue(filterValue))
                return true;
...
function inferSetValue(serialized, value) {
...
export function isFilterStatement(filter) {
    return (filter instanceof Array &&
        filter.length === 3 &&
        typeof filter[0] === 'string' &&
        typeof filter[1] === 'string');
}
export function isIdFilter(filter) {
    return isFilterStatement(filter) && filter[0] === 'id';
}
export function isIdFilterEqualityStatement(filter) {
...
export function isSubQueryFilter(filter) {
    return (filter instanceof Object &&
        'exists' in filter &&
        'collectionName' in filter['exists']);
}
export function isRelationshipExistsFilter(filter) {
    return (filter instanceof Object &&
        'exists' in filter &&
        '_extends' in filter['exists']);
}
export function isBooleanFilter(filter) {
    return typeof filter === 'boolean';
}
export function isWhereFilter(filter) {
    return (isFilterStatement(filter) ||
 ...
 */
export function isIndexableFilter(filter) {
...
function determineFilterType(filter) {
    ...
    if (isBooleanFilter(filter))
        return 'boolean';
    throw new Error(`Filter type could not be determined: ${JSON.stringify(filter)}`);
}
/**
 ...
 */
export function getFilterPriorityOrder(where) {
    if (!where)
        return [];
    const basicFilters = [];
...
export function or(where) {
    return { mod: 'or', filters: where };
}
export function and(where) {
    return { mod: 'and', filters: where };
}
export function exists(relationship, ext = {}) {
    return {
        exists: {
...
export function someFilterStatementsFlat(statements, someFunction) {
    for (const statement of filterStatementIteratorFlat(statements)) {
        if (someFunction(statement))
            return true;
    }
    return false;
}
export function* filterStatementIteratorFlat(statements) {
...
export function isStaticFilter(filter) {
    if (isBooleanFilter(filter))
        return true;
    if (isFilterStatement(filter))
        return true;
