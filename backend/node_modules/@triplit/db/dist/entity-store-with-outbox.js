...
export class EntityStoreWithOutbox {
    storage;
    doubleBuffer;
    store;
    constructor(storage) {
        this.storage = storage;
        this.doubleBuffer = new KVDoubleBuffer(new DurableWriteBuffer(['buf0']), new DurableWriteBuffer(['buf1']));
    ...
    async applyChanges(tx, changes) {
        await this.doubleBuffer.write(tx, changes);
        return changes;
    }
    async applyChangesWithTimestamp(tx, buffer, timestamp, options) {
        const changesToCache = await this.store.applyChangesWithTimestamp(tx, buffer, timestamp, options);
    ...
    async getEntity(storage, collection, id) {
        ...
        const outbox = await this.doubleBuffer.getChangesForEntity(storage, collection, id);
        return applyOverlay(primary, !!outbox?.delete, outbox?.update);
    }
    async getCollectionStats(storage) {
        throw new Error('Method not implemented.');
    }
    async *getEntitiesInCollection(storage, collection) {
        yield* overlayChangesOnCollection(this.store.getEntitiesInCollection(storage, collection), await this.doubleBuffer.getChangesForCollection(storage, collection));
    }
}
//# sourceMappingURL=entity-store-with-outbox.js.map