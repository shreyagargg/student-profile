import { flipOperator, SET_OP_PREFIX, } from '../schema/index.js';
import { IncludedNonRelationError, InvalidCollectionNameError, InvalidFilterError, InvalidOrderClauseError, InvalidQueryCardinalityError, InvalidSelectClauseError, RelationDoesNotExistError, TriplitError, InvalidQueryAfterError, InvalidQueryInclusionError, InvalidQueryLimitError, InvalidQueryWhereError, } from '../errors.js';
import { and, or, exists, isBooleanFilter, isRelationshipExistsFilter, isFilterGroup, isFilterStatement, isSubQueryFilter, } from '../filters.js';
import { getVariableComponents, isValueVariable, isVariableScopeRelational, safeIncrementQueryVars, safeIncrementSubqueryVar, } from '../variables.js';
import { ValuePointer } from '../utils/value-pointer.js';
import { getCollectionPermissions, isWritePermissionOperation, } from '../permissions.js';
import { createSchemaEntriesIterator, getAttributeFromSchema, isTraversalRelationship, validateIdentifier, } from '../schema/utilities.js';
import { simplifyQuery } from './simplify-query.js';
import { Type } from '../schema/data-types/index.js';
import { isQueryInclusionReference, isQueryInclusionShorthand, isQueryInclusionSubquery, isQueryResultCardinality, } from '../subquery.js';
const ACCESS_DENIED_FILTER = Object.freeze([false]);
...
// ): PreparedQuery;
export function prepareQuery(query, schema, variables, session, options) {
    ...
    const preparedQuery = prepareQueryRecursive(query, schema, variables, session, {
...
function prepareQueryRecursive(query, schema, variables, session, options) {
 ...
 */
function prepareCollectionName(query, schema) {
    const collectionName = query.collectionName;
    // Must exist
    if (!collectionName)
        throw new InvalidCollectionNameError(collectionName, 'Collection name must be provided');
    // Must be string
    const isString = typeof collectionName === 'string';
    if (!isString)
        throw new InvalidCollectionNameError(collectionName, 'Collection name must be a string');
    ...
    if (schema) {
        if (!schema[collectionName] && !isSystemCollection(collectionName))
            throw new InvalidCollectionNameError(collectionName, `Collection '${collectionName}' does not exist in the schema`);
    }
 ...
 */
function prepareQuerySelects(query, schema) {
    ...
    for (const select of query.select) {
        ...
        if (typeof select !== 'string')
            throw new InvalidSelectClauseError(`Select field '${select}' is not a string`);
        // If schema is provided, validate that the field exists in the schema
        if (schema) {
            const { valid, path, reason } = validateIdentifier(select, schema, query.collectionName, (dataType, i, path) => {
                if (!dataType)
                    return { valid: false, reason: 'Path not found' };
                if (isTraversalRelationship(dataType)) {
                    return {
                        valid: false,
                        reason: "Cannot select into relationships, please use 'include' instead",
                    };
                }
                return { valid: true };
            });
            if (!valid) {
                throw new InvalidSelectClauseError(`Select field '${select}' is not valid: ${reason} at path '${path}'`);
            }
        }
    }
    return [...query.select];
 ...
 */
function prepareQueryInclude(query, schema, variables, session, options) {
    if (!query.include)
    ...
    for (let [alias, relation] of Object.entries(query.include)) {
        ...
        if (schema) {
            ...
            if (relationName) {
                ...
                const relationDefinition = getAttributeFromSchema(relationName.split('.'), schema, query.collectionName);
                ...
                if (relationDefinition && isTraversalRelationship(relationDefinition)) {
                    const merged = mergeQueries(relationDefinition.query, 
                    ...
                    if (!relationDefinition) {
                        throw new RelationDoesNotExistError(relationName, alias, query.collectionName);
                    }
                    throw new IncludedNonRelationError(relationName, alias, query.collectionName);
                }
            }
        }
        ...
        const preparedQuery = prepareQueryRecursive(relation.subquery, schema, variables, session, options);
        inclusions[alias] = {
 ...
 */
export function prepareQueryFilters(query, schema, variables, session, options) {
    let inputFilters = query.where ?? [];
    if (!Array.isArray(inputFilters))
        throw new InvalidQueryWhereError(query.where, 'Where clause must be an array');
    ...
    // 3. merge the two
    const where = transformAndValidateFilters(inputFilters, query, schema, variables, session, options);
    const shouldApplyPermissions = options.applyPermission &&
    ...
    if (shouldApplyPermissions) {
        const collectionPermissions = getCollectionPermissions(schema, query.collectionName);
        ...
        if (collectionPermissions) {
            ...
            if (session?.roles) {
                for (const sessionRole of session.roles) {
                    const permission = collectionPermissions[sessionRole.key]?.[options.applyPermission];
                    if (permission?.filter) {
                        if (Array.isArray(permission.filter)) {
                            ...
                            if (permission.filter.length === 0) {
                                ...
                                const nextOptions = {
                                    ...options,
                                    applyPermission: isWritePermissionOperation(options.applyPermission)
                                        ? 'read'
                                 ...
                                 */
                                const prefixedPermissionFilters = prependPermissionFilterVariables(permission.filter);
                                permissionFilters.push(transformAndValidateFilters(prefixedPermissionFilters, query, schema, 
                                // Apply role variables
                                { ...variables, $role: sessionRole.roleVars }, session, nextOptions));
                            }
                        }
                        else {
                            throw new InvalidFilterError(`Invalid permission filter format, expected array`);
                        }
                    }
                }
            }
            ...
            const permissionsWhere = or(permissionFilters.map((filters) => and(filters)));
...
function prependPermissionFilterVariables(filters) {
...
function transformAndValidateFilters(filters, query, schema, variables, session, options) {
    return filters.map((filter) => transformAndValidateFilter(filter, query, schema, variables, session, options));
}
/**
 * Transforms and validates individual filters recursively
 */
function transformAndValidateFilter(filter, query, schema, variables, session, options) {
    ...
    if (isFilterGroup(filter)) {
        ...
        return {
            mod: filter.mod,
            filters: transformAndValidateFilters(filter.filters, query, schema, variables, session, options),
        };
    }
    ...
    if (isFilterStatement(filter)) {
        ...
        if (schema) {
            ...
            let propAttributeType;
            // Loop through the path and get the attribute type
            for (const [attrPath, attr] of createSchemaEntriesIterator(propPath, schema, query.collectionName)) {
                ...
                if (isTraversalRelationship(attr)) {
                    ...
                    const filter = transformAndValidateFilter({
                        exists: subquery,
                    }, subquery, schema, variables, session, options);
                    return filter;
                }
            ...
            if (!propAttributeType)
                throw new InvalidFilterError(`Could not find property '${prop}' in the schema`);
            /**
            ...
            if (!Type.supportedOperations(propAttributeType).includes(op))
                throw new InvalidFilterError(`Operator '${op}' is not valid for property '${prop}' of type '${propAttributeType.type}'`);
        }
        // replace variables
        if (isValueVariable(val)) {
            const components = getVariableComponents(val);
            ...
            if (!isVariableScopeRelational(scope)) {
                if (options.replaceStaticVariables) {
                    const variable = ValuePointer.Get(variables, components);
                    /**
            ...
            // Replace relational paths with subquery filter
                 ...
                 * 5. Eval as subquery
                 */
                const refQuery = options.queryStack[options.queryStack.length - scope - 1];
                const valPath = components.slice(1);
                for (const [attrPath, attr] of createSchemaEntriesIterator(valPath, schema, refQuery.collectionName)) {
                    if (isTraversalRelationship(attr)) {
                        const subquery = { ...attr.query };
                        // Placing the subquery in a nested spot, increment variables based on how deep we are in the query stack
                        const incrementedSubquery = safeIncrementQueryVars(subquery, options.queryStack.length - 1);
    ...
    if (isRelationshipExistsFilter(filter)) {
        if (!schema)
            throw new InvalidFilterError('A schema is required to execute an exists filter');
        const { exists: _exists } = filter;
        const { _extends: relationship, ...queryExt } = _exists;
        const relationshipPath = relationship.split('.');
        let queryPath;
        let relationshipAttributeType;
        // Loop through the path and find the first query type
        for (const [relPath, rel] of createSchemaEntriesIterator(relationshipPath, schema, query.collectionName)) {
            queryPath = relPath;
        ...
        if (!relationshipAttributeType)
            throw new InvalidFilterError(`Could not find property '${relationship}' in the schema`);
        if (!isTraversalRelationship(relationshipAttributeType))
            throw new InvalidFilterError('Cannot execute an exists filter on a non-relation property');
        const subquery = { ...relationshipAttributeType.query };
    ...
    if (isSubQueryFilter(filter)) {
        ...
        return {
            exists: prepareQueryRecursive(subquery, schema, variables, session, options),
        };
    }
 ...
 */
function prepareQueryOrder(query, schema, variables, session, options) {
    if (!query.order)
        return query.order;
    const order = [];
    for (const [field, direction] of query.order) {
        if (direction !== 'ASC' && direction !== 'DESC') {
            throw new InvalidOrderClauseError(`Invalid order direction '${direction}'`);
        }
        // Validate that the order by fields exist and are sortable
        if (schema) {
            const { valid, path, reason } = validateIdentifier(field, schema, query.collectionName, (dataType, i, path) => {
                if (!dataType)
                    return { valid: false, reason: 'Path not found' };
                if (i === path.length - 1 &&
                ...
                if (isTraversalRelationship(dataType) &&
                    ...
                    return {
                        valid: false,
                        reason: 'Order by field is a query with cardinality not equal to one',
                    };
                return { valid: true };
            });
            if (!valid) {
                throw new InvalidOrderClauseError(`Order by field '${field}' is not valid: ${reason} at path '${path}'`);
            }
            let subqueryRef = undefined;
            let cardinality = undefined;
            for (const [path, dataType] of createSchemaEntriesIterator(field.split('.'), schema, query.collectionName)) {
            ...
            if (subqueryRef) {
                const prepared = prepareQueryRecursive(subqueryRef, schema, variables, session, options);
                order.push([
                    field,
                    direction,
 ...
 */
function prepareQueryLimit(query) {
    if (!query.limit)
        return query.limit;
...
function prepareQueryAfter(query, schema) {
    if (!query.after)
        return query.after;
    ...
    if (query.after.length < 2)
        throw new InvalidQueryAfterError('After clause must contain a cursor and inclusive flag');
    if (query.after.length > 2)
        throw new InvalidQueryAfterError('After clause has too many components - it should contain a cursor and inclusive flag');
    if (!Array.isArray(query.after[0]))
        throw new InvalidQueryAfterError('After clause cursor must be an array');
    if (!query.order)
        throw new InvalidQueryAfterError('After clause requires an order by clause to be provided');
    if (query.after[0].length !== query.order.length)
        throw new InvalidQueryAfterError('After clause cursor must match the elements in the order by clause');
    if (typeof query.after[1] !== 'boolean')
        throw new InvalidQueryAfterError('After clause inclusive flag must be a boolean');
    // @ts-expect-error
    const cursor = [];
    ...
    for (let i = 0; i < query.order.length; i++) {
        ...
        if (schema) {
            ...
            for (const [attrPath, attr] of createSchemaEntriesIterator(orderPath, schema, query.collectionName)) {
                if (!attr)
                    throw new InvalidQueryAfterError(`Attribute ${query.order[i][0]} not found in schema`);
                ...
                if (isTraversalRelationship(attr)) {
                    throw new InvalidQueryAfterError(`Attribute ${query.order[i][0]} is not allowed in after clause because it is relational`);
                }
                ...
                if (isLeaf) {
                    ...
                    if (attr.type === 'set' ||
                        ...
                        attr.type === 'record') {
                        throw new InvalidQueryAfterError(`Attribute ${query.order[i][0]} is not allowed in after clause because the type is not sortable`);
                    }
                    ...
                    try {
                        ...
                        if (e instanceof TriplitError)
                            throw new InvalidQueryAfterError(`Error converting cursor value for attribute ${query.order[i][0]}: ${e.message}`);
                        throw e;
                    }
                }
            }
...
//# sourceMappingURL=prepare-query.js.map