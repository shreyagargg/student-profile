...
import { isBooleanFilter, isFilterGroup, isFilterStatement, isSubQueryFilter, } from '../filters.js';
import { TriplitError } from '../errors.js';
import { encodingByte } from '../codec.js';
 ...
 */
export function hashQuery(params) {
    const queryParams = Object.fromEntries(Object.entries(params).filter(([key]) => COLLECTION_QUERY_PROPS.includes(key)));
...
export function hashPreparedQuery(query) {
...
export function hashFilter(filter) {
    ...
    throw new TriplitError('Could not hash filter, it is not a valid filter type');
}
const FILTER_PREFIX = fnv1aHash('F');
export function hashFilters(filters) {
    let h = FILTER_PREFIX;
    const filterHashes = filters.map(hashFilter);
    filterHashes.sort((a, b) => a.sortKey < b.sortKey ? -1 : a.sortKey > b.sortKey ? 1 : 0);
...
export function hashInclusions(include) {
    let ih = INCLUSION_PREFIX;
    const sorted = Object.entries(include).sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0);
...
export function hashAfter(after) {
    let ah = AFTER_PREFIX;
...
export function hashLimit(limit) {
    let lh = LIMIT_PREFIX;
...
export function encodeHashValue(value) {
    if (value === undefined) {
...
//# sourceMappingURL=hash-query.js.map