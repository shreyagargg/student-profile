...
function simplifyWhereClauses(where, groupWith) {
    let clauses = [];
...
function deduplicateWhereClauses(where, groupWith) {
    const seen = new Set();
    const deduplicated = [];
 ...
 */
function simplifyWhereClause(clause) {
 ...
 */
export function simplifyFilterGroup(filterGroup) {
    // Simplify the filter group filters
    filterGroup.filters = simplifyWhereClauses(filterGroup.filters, filterGroup.mod);
 ...
 */
function applyBooleanCollapse(filters, groupWith) {
    if (groupWith === 'and') {
...
function simplifyInclusions(inclusions) {
    if (!inclusions)
        return inclusions;
...
//# sourceMappingURL=simplify-query.js.map