import { DBInitializationError, DBSerializationError, TriplitError, WritePermissionError, } from './errors.js';
...
import { getSchemaDiffIssues, diffSchemas, } from './schema/diff-issues.js';
import { prepareQuery } from './query/prepare-query.js';
import { createSession, normalizeSessionVars } from './session.js';
import { getTypeConvertersFromSchema, } from './schema/converters.js';
...
export class DB {
    ...
    constructor(options = {}) {
        ...
        if (options.schema) {
            ...
            if (schemaInvalid)
                throw new DBInitializationError(`Failed to start DB because the current schema is invalid: ${schemaInvalid}`);
        }
        this.schema = options.schema;
        this.typeConverters = getTypeConvertersFromSchema(this.schema);
    }
    static async getSchemaFromStorage(kv) {
    ...
    options = {}) {
        ...
        const callback = ({ results }) => {
            onResults(applyProjectionsAndConversions(results, preparedQuery, 'many', this.typeConverters), options.queryKey);
        };
        return this.ivm.subscribe(preparedQuery, callback, onError);
    }
    subscribeRaw(query, onResults, onError, options = {}) {
        const callback = ({ results }) => {
     ...
     */
    subscribeWithChanges(query, onResults, onError, options = {}) {
        const preparedQuery = prepareQuery(query, this.schema?.collections, this.systemVars, this.session, {
            applyPermission: options.skipRules ? undefined : 'read',
        });
        const callback = ({ results, changes }) => {
            onResults({
                results: applyProjectionsAndConversions(results, preparedQuery, 'many', this.typeConverters),
                changes,
            });
        };
        return this.ivm.subscribe(preparedQuery, callback, onError);
     ...
     * @deprecated TODO remove
     */
    subscribeChanges(query, onResults, options = {}) {
        const preparedQuery = prepareQuery(query, this.schema?.collections, this.systemVars, this.session, {
            applyPermission: options.skipRules ? undefined : 'read',
        });
        ...
        const callback = async ({ changes }) => {
            ...
            if (options.queryState && isInitialResponse) {
                ...
                const entitiesThatHaveNotChanged = {};
                const entitiesThatAreNoLongerInTheResultSet = {};
                for (const [collection, entityIds] of Object.entries(options.queryState.entityIds)) {
                ...
                for (const [collectionName, entityIds] of Object.entries(entitiesThatAreNoLongerInTheResultSet)) {
     ...
     * @returns
     */
    async fetchChanges(query, options) {
        const preparedQuery = prepareQuery(query, this.schema?.collections, this.systemVars, this.session, {
            applyPermission: options?.skipRules ? undefined : 'read',
        });
        const results = (await this.rawFetch(preparedQuery));
    ...
    async applyChangesWithTimestamp(changes, timestamp, options) {
        const kvTx = this.kv.transact();
        const appliedChanges = await this.entityStore.applyChangesWithTimestamp(kvTx, changes, timestamp, {
            checkWritePermission: options?.skipRules
                ? undefined
                : this.checkWritePermission.bind(this),
            entityChangeValidator: this.validateEntityChange.bind(this),
    ...
    async getCollectionStats() {
        return this.entityStore.getCollectionStats(this.kv, this.schema ? Object.keys(this.schema.collections) : undefined);
    }
    async overrideSchema(newSchema, options) {
    ...
    async _overrideSchema(newSchema, { failOnBackwardsIncompatibleChange = false } = {}) {
        ...
        issues = await getSchemaDiffIssues((query) => this.fetch.bind(this)(query, { skipRules: true }), diff);
    ...
    async updateSchema(schema) {
         ...
         */
        await this.entityStore.dataStore.applyChanges(storageTx, {
            _metadata: {
                // Perform a set AND delete to ensure a full overwrite and no merging
                sets: new Map([['_schema', { id: '_schema', ...schema }]]),
    ...
    onSchemaChange(callback) {
        ...
        return () => {
            this.schemaChangeListeners = this.schemaChangeListeners.filter((listener) => listener !== callback);
        };
    }
    ...
    validateEntityChange(collection, change, { ignoreRequiredProperties = false }) {
        ...
        if (!validation.valid) {
            throw new DBSerializationError(`record`, JSON.stringify(change), validation.error);
        }
    }
    ...
    async checkWritePermission(storage, delta, operation) {
        ...
        const permissions = getCollectionPermissions(this.schema?.collections, collection);
        ...
        }, this.schema?.collections, { ...this.systemVars, $prev: delta.prev }, this.session, {
        ...
        if (!preparedPermissions || preparedPermissions.length === 0)
            return;
        ...
        const isSatisfied = await satisfiesFilters(entity, preparedPermissions, queryEngine);
        if (!isSatisfied) {
            throw new WritePermissionError(collection, entity.id, operation, this.session?.roles ?? []);
        }
    }
}
...
export function changesToTriples(changes, timestamp) {
    let triples = [];
    for (const [collectionName, collectionChanges] of Object.entries(changes)) {
        for (const [entityId, change] of collectionChanges.sets) {
            triples = triples.concat(recursivelyGetTriplesFromObj(change, entityId, collectionName, [], timestamp));
        }
    }
    return triples;
}
function recursivelyGetTriplesFromObj(obj, id, collection, attribute, timestamp) {
    if (obj === null || typeof obj !== 'object' || Array.isArray(obj)) {
        return [{ id, attribute, collection, value: obj, timestamp }];
    }
    const triples = [];
    for (const [key, value] of Object.entries(obj)) {
        triples.push(...recursivelyGetTriplesFromObj(value, id, collection, [...attribute, key], timestamp));
    }
    return triples;
}
/**
    ...
    try {
        ...
        if (options.schema) {
            schemaChange = await db.overrideSchema(options.schema, {
                failOnBackwardsIncompatibleChange: options.failOnBackwardsIncompatibleChange,
            });
        ...
        if (db) {
            return { db, event: { type: 'ERROR', error: error } };
        }
        else {
            throw new TriplitError(`Failed to create a DB instance: ${error.message}`);
        }
    }
}
function entityIsInChangeset(changes, collection, entityId) {
    return (changes[collection] &&
        (changes[collection].sets.has(entityId) ||
            changes[collection].deletes.has(entityId)));
}
export function applyProjectionsAndConversions(results, query, cardinality, typeConverters) {
    if (results === null)
        return null;
    const dataConverter = typeConverters?.get(query.collectionName);
    const convertEntity = (entityData) => {
    ...
    const projectAndConvertEntity = (entity) => {
        ...
        const convertedInclusions = query.include &&
            Object.entries(query.include).reduce((acc, [key, inclusion]) => {
                if (entity.subqueries[key] !== undefined) {
                    acc[key] = applyProjectionsAndConversions(entity.subqueries[key], inclusion.subquery, inclusion.cardinality, typeConverters);
                }
                return acc;
            }, {});
        return {
    ...
    return cardinality === 'one'
        ...
        : results.map(projectAndConvertEntity);
}
