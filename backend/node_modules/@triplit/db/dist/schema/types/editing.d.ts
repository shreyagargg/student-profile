...
export type Diff = CollectionAttributeDiff | CollectionRelationshipsDiff | CollectionPermissionsDiff | RolesDiff;
export type ALLOWABLE_DATA_CONSTRAINTS = 'none' | 'never' | 'collection_is_empty' | 'attribute_is_empty' | 'attribute_has_no_undefined' | 'attribute_has_no_null' | 'attribute_satisfies_enum';
export type BackwardsIncompatibleEdit = {
    issue: string;
    dataConstraint: ALLOWABLE_DATA_CONSTRAINTS;
    context: CollectionAttributeDiff;
    attributeCure: (collection: string, attribute: string[], enums?: string[]) => string | null;
};
...
export type FailedSchemaChange = SchemaChangeBase & {
    successful: false;
    code: 'SCHEMA_INVALID' | 'SCHEMA_COMPATIBILITY_MISMATCH' | 'EXISTING_DATA_MISMATCH' | 'ERROR';
};
