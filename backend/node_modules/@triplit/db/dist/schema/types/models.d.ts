...
import { QueryResultCardinality, QueryWhere, SchemaQuery } from '../../query/types/index.js';
...
export type Models<T = Record<string, Collection<any>>> = {
    [K in StringKey<T>]: Collection<T, K>;
};
/**
 * Information pertaining to a collection
 */
export interface Collection<M extends Models<M> = Models, CN extends CollectionNameFromModels<M> = CollectionNameFromModels<M>> {
    schema: Model;
 ...
 */
type ModelRelationship<M extends Models<M> = Models, CN extends CollectionNameFromModels<M> = CollectionNameFromModels<M>> = ModelsRelationship<M>[CN];
export type Relationships<M extends Models<M> = Models, CN extends CollectionNameFromModels<M> = CollectionNameFromModels<M>> = {
    [R in string]: ModelRelationship<M, CN>;
};
/**
 * A relationship between two collections
 */
export type Relationship<M extends Models<M> = Models, Q extends SchemaQuery<M> = SchemaQuery<M>, Cardinality extends QueryResultCardinality = QueryResultCardinality> = {
    query: Q;
 ...
 */
type ModelRolePermissions<M extends Models<M> = Models, CN extends CollectionNameFromModels<M> = CollectionNameFromModels<M>> = ModelsRolePermissions<M>[CN];
/**
 * A collection of permissions by role
 */
export type RolePermissions<M extends Models<M> = Models, CN extends CollectionNameFromModels<M> = CollectionNameFromModels<M>> = {
    [R in string]: CollectionPermissions<M, CN>;
};
/**
 * Permissions for a collection by known operation
 */
export type CollectionPermissions<M extends Models<M> = Models, CN extends CollectionNameFromModels<M> = CollectionNameFromModels<M>> = {
 ...
 */
export type CollectionPermission<M extends Models<M> = Models, CN extends CollectionNameFromModels<M> = CollectionNameFromModels<M>> = {
...
export interface CollectionRules<M extends Models<M>, CN extends CollectionNameFromModels<M>> {
    read?: Record<string, Rule<M, CN>>;
    write?: Record<string, Rule<M, CN>>;
}
export interface Rule<M extends Models<M>, CN extends CollectionNameFromModels<M>> {
    filter: QueryWhere<M, CN>;
    description?: string;
}
export {};
