...
import { DBDeserializationError, DBSerializationError, JSONDeserializationError, JSONSerializationError, TriplitError, UnrecognizedAttributeTypeError, } from '../../errors.js';
...
export function defaultValue(type) {
    if (hasConfigurableDefault(type))
        return calcDefaultValue(type.config);
    if (type.type === 'record') {
        const result = {};
        for (const key in type.properties) {
...
export function assign(type, target, input) {
    switch (type.type) {
        case 'record':
            for (const key in type.properties) {
    ...
    switch (type.type) {
        ...
        case 'record':
            if (typeof input === 'object' && input !== null) {
                for (const key in input) {
                    const property = type.properties[key];
                    if (!property) {
                        throw new DBSerializationError('record', input, `Unrecognized property: ${key}`);
                    }
                    ...
                    try {
                        ...
                        if (e instanceof DBSerializationError) {
                            throw new DBSerializationError('record', input, `Could not encode property: '${key}'. Error: ${e.message}`);
                        }
                        throw e;
                    }
                }
        ...
        case 'string':
            if (typeof input === 'string') {
                if (type.config?.enum) {
                    if (type.config?.enum.includes(input))
                        return input;
                }
                else {
                    return input;
        ...
        case 'set':
            if (Array.isArray(input))
                return Object.fromEntries(input.map((v) => [collectionKeyEncode(type.items, v), true]));
            if (input instanceof Set)
                return Object.fromEntries(Array.from(input).map((v) => [
    ...
    switch (type.type) {
        ...
        case 'record':
            for (const key in type.properties) {
                ...
                if (options.partial && !(key in encoded))
                    continue;
    ...
    switch (type.type) {
            ...
            for (const key in encoded) {
                ...
                try {
                    ...
                    if (e instanceof DBDeserializationError) {
                        throw new DBDeserializationError('record', encoded, `Could not decode property: '${key}'. Error: ${e.message}`);
                    }
                    throw e;
                }
            }
        ...
        case 'set':
            if (typeof encoded === 'object')
                return new Set(Object.entries(encoded)
                    .filter(([_, v]) => v)
...
export function collectionKeyEncode(type, input) {
    if (!isPrimitiveType(type))
        throw new TriplitError('Cannot encode collection key for non-primitive type');
    return encode(type, input).toString();
}
// From string
export function collectionKeyDecode(type, encoded) {
    if (!isPrimitiveType(type))
        throw new TriplitError('Cannot decode collection key for non-primitive type');
    if (type.type === 'number')
...
export function serialize(type, input, inputFormat) {
    if (type.type === 'date') {
        if (inputFormat === 'encoded')
            return input;
        if (inputFormat === 'decoded')
            return encode(type, input);
    ...
    if (type.type === 'record') {
        ...
        for (const key in input) {
            const property = type.properties[key];
            if (!property)
                throw new JSONSerializationError('record', input, `Unrecognized property: ${key}`);
            if (isOptional(property) && hasNoValue(input[key]))
                continue;
    ...
    if (type.type === 'set') {
        if (inputFormat === 'encoded') {
            return Object.entries(input)
                .filter(([_, v]) => v)
                .map(([k, _]) => serialize(type.items, collectionKeyDecode(type.items, k), inputFormat));
        }
        if (inputFormat === 'decoded') {
            const serialized = [];
...
export function deserialize(type, input, outputFormat) {
    if (type.type === 'date') {
    ...
    if (type.type === 'record') {
        for (const key in input) {
            const property = type.properties[key];
            if (!property)
                throw new JSONDeserializationError('record', input, `Unrecognized property: ${key}`);
            if (isOptional(property) && hasNoValue(input[key]))
                continue;
...
export function supportedOperations(type) {
    if (type.type === 'boolean')
        return SUPPORTED_OPERATIONS.boolean;
    if (type.type === 'date')
        return SUPPORTED_OPERATIONS.date;
    if (type.type === 'json')
        return SUPPORTED_OPERATIONS.json;
    if (type.type === 'number')
        return SUPPORTED_OPERATIONS.number;
    if (type.type === 'record')
        return SUPPORTED_OPERATIONS.record;
    if (type.type === 'set')
        return [
 ...
 */
export function hasConfigurableDefault(type) {
    return DEFAULTABLE_TYPE_KEYS_SET.has(type.type);
}
export function isPrimitiveType(type) {
    return PRIMITIVE_TYPE_KEYS_SET.has(type.type);
}
function calcDefaultValue(config) {
...
function encodedValueMismatchMessage(type, value) {
...
function setEqual(a, b) {
    if (!typeConfigEqual(a.config, b.config))
        return false;
...
function stringEqual(a, b) {
    ...
    if (!a.config?.enum && b.config?.enum)
        return false;
    if (a.config?.enum && b.config?.enum) {
        if (a.config.enum.length !== b.config.enum.length)
            return false;
        for (const value of a.config.enum) {
            if (!b.config.enum.includes(value))
                return false;
        }
    }
    return typeConfigEqual(a.config, b.config);
...
function typeConfigEqual(a, b) {
    if (!!a?.optional !== !!b?.optional)
        return false;
    if (!!a?.nullable !== !!b?.nullable)
        return false;
    if (!typeDefaultEqual(a?.default, b?.default))
        return false;
    return true;
}
function typeDefaultEqual(a, b) {
...
function defaultFunctionEqual(a, b) {
    if (a.func !== b.func)
        return false;
