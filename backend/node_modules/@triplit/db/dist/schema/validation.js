...
function validateRoles(roles) {
    ...
    if (typeof roles !== 'object')
        return 'roles is not an object';
    for (const key in roles) {
        const invalidRoleName = validateRoleName(key);
        if (invalidRoleName)
            return `role "${key}" is invalid: ${invalidRoleName}`;
        const role = roles[key];
        const invalidRole = validateRole(role);
        if (invalidRole)
            return `role "${key}" is invalid: ${invalidRole}`;
    }
}
function validateRole(role) {
    ...
    if (typeof role !== 'object')
        return 'role is not an object';
    if (hasNoValue(role.match))
        return 'matcher is not defined';
    if (typeof role.match !== 'object')
        return 'matcher is not an object';
}
function validateCollections(collections, schema) {
    if (hasNoValue(collections))
        return 'collections is not defined';
...
export function validateCollectionName(name) {
    ...
    if (typeof name !== 'string')
        return 'collection name is not a string';
    if (name.length === 0)
...
export function validateCollection(collection, schema) {
    if (hasNoValue(collection))
        return 'collection is not defined';
    if (typeof collection !== 'object')
        return 'collection is not an object';
    ...
    if ('relationships' in collection) {
        const invalidRelationships = validateRelationships(collection.relationships, collection);
        if (invalidRelationships)
            return `collection relationships is invalid: ${invalidRelationships}`;
    }
    if ('permissions' in collection) {
        const invalidPermissions = validateRolePermissions(collection.permissions, schema);
...
export function validateCollectionSchema(schema) {
    ...
    if (invalidId)
        return `primary key field "id" is invalid: ${invalidId}`;
}
export function validateDataType(type) {
    if (hasNoValue(type))
        return 'type is not defined';
    if (typeof type !== 'object')
        return 'type is not a DataType';
...
function validateBooleanType(type) {
    if (type.type !== 'boolean')
        return 'not a boolean type';
    const configInvalid = validateBaseDataTypeConfig(type);
    if (configInvalid)
        return `type boolean is invalid: ${configInvalid}`;
    return;
}
function validateDateType(type) {
    if (type.type !== 'date')
        return 'not a date type';
    const configInvalid = validateBaseDataTypeConfig(type);
    if (configInvalid)
        return `type date is invalid: ${configInvalid}`;
    return;
}
function validateJsonType(type) {
    if (type.type !== 'json')
        return 'not a json type';
    const configInvalid = validateBaseDataTypeConfig(type);
    if (configInvalid)
        return `type json is invalid: ${configInvalid}`;
    return;
}
function validateNumberType(type) {
    if (type.type !== 'number')
        return 'not a number type';
    const configInvalid = validateBaseDataTypeConfig(type);
    if (configInvalid)
        return `type number is invalid: ${configInvalid}`;
    return;
}
function validateRecordType(type) {
    ...
    const configInvalid = validateBaseDataTypeConfig(type);
    if (configInvalid)
        return `type record is invalid: ${configInvalid}`;
    if (hasNoValue(type.properties))
        return 'type record is missing properties';
...
function validateSetType(type) {
    if (type.type !== 'set')
        return 'not a set type';
    const configInvalid = validateBaseDataTypeConfig(type);
    if (configInvalid)
        return `type set is invalid: ${configInvalid}`;
    if (hasNoValue(type.items))
        return 'type set is missing items';
    const invalidItems = validateDataType(type.items);
    if (invalidItems)
        return `type set items is invalid: ${invalidItems}`;
    if (!isPrimitiveType(type.items))
        return 'type set items must be a primitive type';
}
function validateStringType(type) {
    if (type.type !== 'string')
        return 'not a string type';
    const configInvalid = validateBaseDataTypeConfig(type);
    if (configInvalid)
        return `type string is invalid: ${configInvalid}`;
...
export function validateBaseDataTypeConfig(type) {
    if (hasNoValue(type.config))
        return;
    if (typeof type.config !== 'object')
        return 'type config is not an object';
    // validate nullable
    if (!hasNoValue(type.config.nullable) &&
        typeof type.config.nullable !== 'boolean')
        return 'option nullable is invalid';
    // validate optional
    if (!hasNoValue(type.config.optional) &&
        typeof type.config.optional !== 'boolean')
        return 'option optional is invalid';
    // validate default
    const invalidDefault = validateDefaultValue(type);
    if (invalidDefault)
        return `option default is invalid: ${invalidDefault}`;
}
function validateDefaultValue(type) {
    const defaultValue = type.config?.default;
    if (hasNoValue(defaultValue))
        return;
    if (type.type === 'record')
    ...
    if (isDefaultFunction(defaultValue)) {
        const invalidDefaultFunction = validateDefaultFunction(type.type, defaultValue);
...
function validateDefaultFunction(type, fnObj) {
    if (typeof fnObj !== 'object')
        return 'default function format is invalid';
    ...
    if (funcId === 'now') {
        if (!hasNoValue(fnObj.args))
            return 'default function "now" has no args';
...
function isDefaultFunctionId(id) {
    return DEFAULT_FUNCTIONS.includes(id);
}
function validateRelationships(relationships, collection) {
    if (hasNoValue(relationships))
        return;
...
function validateRelationshipName(name, collection) {
    if (hasNoValue(name))
        return 'relationship name is not defined';
    if (typeof name !== 'string')
        return 'relationship name is not a string';
    if (name.length === 0)
        return 'relationship name is empty';
    if (name.startsWith('_'))
...
function validateRelationship(relationship) {
    if (hasNoValue(relationship))
        return 'relationship is not defined';
    if (typeof relationship !== 'object')
        return 'relationship is not an object';
    if (hasNoValue(relationship.cardinality))
        return 'cardinality is not defined';
    if (relationship.cardinality !== 'one' && relationship.cardinality !== 'many')
        return 'cardinality is invalid';
    if (hasNoValue(relationship.query))
        return 'query is not defined';
    if (typeof relationship.query !== 'object')
        return 'query is not an object';
    // Additional query validation occurs in prepareQuery
}
function validateRolePermissions(permissions, schema) {
    if (hasNoValue(permissions))
        return;
...
function validatePermissionOperations(collectionPermissions) {
    // technically we'll allow, though i dont think we're typed to allow
    if (hasNoValue(collectionPermissions))
        return;
    if (typeof collectionPermissions !== 'object')
        return 'not an object';
    if (collectionPermissions.read) {
        const invalidPermission = validateCollectionPermission(collectionPermissions.read);
        if (invalidPermission)
            return `"read" permission is invalid: ${invalidPermission}`;
    }
    if (collectionPermissions.insert) {
        const invalidPermission = validateCollectionPermission(collectionPermissions.insert);
        if (invalidPermission)
            return `insert permission is invalid: ${invalidPermission}`;
    }
    if (collectionPermissions.update) {
        const invalidPermission = validateCollectionPermission(collectionPermissions.update);
        if (invalidPermission)
            return `update permission is invalid: ${invalidPermission}`;
    }
    if (collectionPermissions.postUpdate) {
        const invalidPermission = validateCollectionPermission(collectionPermissions.postUpdate);
        if (invalidPermission)
            return `postUpdate permission is invalid: ${invalidPermission}`;
    }
    if (collectionPermissions.delete) {
        const invalidPermission = validateCollectionPermission(collectionPermissions.delete);
        if (invalidPermission)
            return `delete permission is invalid: ${invalidPermission}`;
    }
    return;
}
function validateCollectionPermission(permission) {
    if (hasNoValue(permission))
        return;
    if (typeof permission !== 'object')
        return 'permission is not an object';
    if (hasNoValue(permission.filter))
        return;
...
function validateRoleName(name) {
    ...
    if (typeof name !== 'string')
        return 'role name is not a string';
    if (name.length === 0)
        return 'role name is empty';
    if (name.startsWith('_'))
        return 'role name cannot start with an underscore';
    if (/^[0-9]/.test(name))
