...
function isCollectionAttributeDiff(diff) {
    return diff._diff === 'collectionAttribute';
}
function diffCollectionSchemas(modelA, modelB, attributePathPrefix = []) {
    ...
    for (const prop of allProperties) {
        ...
        if (prop in propertiesA && prop in propertiesB) {
            if (Type.equal(propertiesA[prop], propertiesB[prop]))
                continue;
            const path = [...attributePathPrefix, prop];
            if (propertiesA[prop].type === 'record' &&
                propertiesB[prop].type === 'record') {
                diff.push(...diffCollectionSchemas(propertiesA[prop], propertiesB[prop], path));
                continue;
            }
            const attrDiff = {
                type: 'update',
            ...
            attrDiff.changes.config = diffAttributeOptions(propertiesA[prop].config ?? {}, propertiesB[prop].config ?? {});
            diff.push(attrDiff);
            continue;
        }
    }
...
function diffAttributeOptions(attr1, attr2) {
    const diff = {};
    ...
    const removedAnEnumOption = attr1.enum &&
        attr2.enum &&
...
export function diffSchemas(schemaA, schemaB) {
    ...
    for (const collection of allCollections) {
        ...
        // Diff schemas
        diff.push(...diffCollectionSchemas(collectionA?.schema, collectionB?.schema).map((change) => ({
            _diff: 'collectionAttribute',
            collection,
            ...change,
        })));
        const isRelationDiff = !relationshipsEqual(collectionA?.relationships, collectionB?.relationships);
        if (isRelationDiff)
            diff.push({
        ...
        // Diff permissions
        const isPermissionDiff = !permissionsEqual(collectionA?.permissions, collectionB?.permissions);
        if (isPermissionDiff)
            diff.push({
...
function relationshipsEqual(a, b) {
    ...
    for (const key of keysA) {
        ...
        if (!relationshipEqual(relationshipA, relationshipB))
            return false;
    }
    return true;
}
function relationshipEqual(a, b) {
    if (!a && !b)
        return true;
    if (!a || !b)
        return false;
    ...
    if (hashQuery(a.query) !== hashQuery(b.query))
        return false;
    return true;
}
...
// }
export function getBackwardsIncompatibleEdits(schemaDiff) {
    return schemaDiff.reduce((acc, curr) => {
        if (!isCollectionAttributeDiff(curr))
            return acc;
        const maybeDangerousEdit = DANGEROUS_EDITS.find((check) => check.matchesDiff(curr));
...
const DANGEROUS_EDITS = [
    ...
    // },
    {
        ...
        attributeCure: (_collection, attribute) => `make '${attribute.join('.')}' optional`,
    },
    {
    ...
    },
    {
        ...
        dataConstraint: 'attribute_is_empty',
        attributeCure: (_collection, attribute) => `revert the change to '${attribute.join('.')}' and create a different, optional, attribute with the new type`,
    },
    {
    ...
    {
        description: 'added an enum to an attribute or removed an option from an existing enum',
        matchesDiff: (diff) => {
            if (diff.type === 'update') {
        ...
        attributeCure: (_collection, attribute, enumArray) => `revert the change to '${attribute.join('.')}' and create a different, optional, attribute with the new enum OR ensure all values of '${attribute.join('.')} are in the new enum: ${enumArray}`,
    },
];
async function isEditSafeWithExistingData(fetchHandler, attributeDiff, allowedIf) {
    return await DATA_CONSTRAINT_CHECKS[allowedIf](fetchHandler, attributeDiff.collection, attributeDiff.attribute, attributeDiff?.type === 'update'
        ? attributeDiff.changes.config?.enum
        : undefined);
}
export async function getSchemaDiffIssues(fetchHandler, schemaDiff) {
    const backwardsIncompatibleEdits = getBackwardsIncompatibleEdits(schemaDiff);
    // TODO: check promise.all reliability
    const results = await Promise.all(backwardsIncompatibleEdits.map(async (edit) => {
        const violatesExistingData = !(await isEditSafeWithExistingData(fetchHandler, edit.context, edit.dataConstraint));
        const dataCure = edit.dataConstraint &&
            DATA_CHANGE_CURES[edit.dataConstraint](edit.context.collection, edit.context.attribute);
        const attributeCure = edit.attributeCure(edit.context.collection, edit.context.attribute);
        return {
            ...edit,
            violatesExistingData,
            cure: attributeCure && dataCure
...
const DATA_CHANGE_CURES = {
    never: () => 'This edit is never allowed',
    none: () => 'This edit does not violate any data constraints but can cause existing queries that reference this attribute to fail',
    collection_is_empty: (collection) => `delete all entities in '${collection}' to allow this edit`,
    attribute_is_empty: (_collection, attribute) => `set all values of '${attribute.join('.')}' to undefined to allow this edit`,
    attribute_has_no_undefined: (_collection, attribute) => `ensure all values of '${attribute.join('.')}' are not undefined to allow this edit`,
    attribute_has_no_null: (_collection, attribute) => `ensure all values of '${attribute.join('.')}' are not null to allow this edit`,
    attribute_satisfies_enum: (_collection, attribute) => `ensure all values of '${attribute.join('.')}' are in the enum to allow this edit`,
};
async function detectAttributeSatisfiesEnum(fetchHandler, collectionName, attribute, enumArray) {
...
async function detectAttributeHasNoUndefined(fetchHandler, collectionName, attribute) {
    const allEntities = await fetchHandler({
        collectionName,
        select: ['id'],
        limit: 1,
...
async function detectAttributeIsEmpty(fetchHandler, collectionName, attribute) {
    const allEntities = await fetchHandler({
        collectionName,
        select: ['id'],
        limit: 1,
...
async function detectAttributeHasNoNull(fetchHandler, collectionName, attribute) {
    const allEntities = await fetchHandler({
        collectionName,
        select: ['id'],
        limit: 1,
...
async function detectCollectionIsEmpty(fetchHandler, collectionName) {
    const allEntities = await fetchHandler({ collectionName, select: ['id'], limit: 1 }
    // { skipRules: true }
    );
    return allEntities.length === 0;
}
export function logSchemaChangeViolations(change, { logger, forcePrintIssues = false, } = {}) {
    ...
    if (!change.successful || forcePrintIssues) {
        const problematicIssues = change.issues.filter((issue) => forcePrintIssues || issue.violatesExistingData);
        log.error([success, numberOfIssues, constructSchemaIssues(problematicIssues)].join('\n\n'));
    }
}
function constructSchemaIssues(issues) {
    ...
    collectionIssueMap.forEach((issues, collection) => {
        ...
        issues.forEach(({ issue, context, cure }) => {
            message.push(`\t'${context.attribute.join('.')}'
\t\tIssue: ${issue}
\t\tFix:   ${cure}`);
        });
    });
...
//# sourceMappingURL=diff-issues.js.map