...
export const encodingRank = new Map(Object.entries(encodingByte)
    .sort((a, b) => {
    return a[1] < b[1] ? -1 : 1;
})
    .map(([key], i) => [key, i]));
...
export function encodingTypeOf(value) {
    if (value === null) {
        return 'null';
    }
    if (value === true || value === false) {
...
const decodeType = Object.fromEntries(Object.entries(encodingByte).map(([key, value]) => [value, key]));
export function decodeValue(str, options) {
...
export function encodeTuple(tuple, options) {
...
export function decodeTuple(str, options) {
    if (str === '') {
        return [];
    }
    const delimiter = options?.delimiter ?? '\x01';
