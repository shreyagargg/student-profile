...
export class VariableAwareCache {
    static canCacheQuery(query) {
        ...
        if (orderableStatements.length === 0)
            return false;
        return true;
    }
    static resolveQueryFromView(viewResults, [prop, op, val]) {
        let start, end;
        if (['=', '<', '<=', '>', '>='].includes(op)) {
            start = binarySearch(viewResults, val, (ent) => ent.data[prop], 'start', safeCompareValue);
            end = binarySearch(viewResults, val, (ent) => ent.data[prop], 'end', safeCompareValue);
        }
        if (op === '!=') {
            start = binarySearch(viewResults, val, (ent) => ent.data[prop], 'start', safeCompareValue);
            end = binarySearch(viewResults, val, (ent) => ent.data[prop], 'end', safeCompareValue);
        ...
        if (start == undefined || end == undefined) {
            throw new QueryCacheError(`Queries with the operator "${op}" in a where clause can't be stored in the query cache. Currently, supported operators are: ${[
                '=',
                '!=',
                '<',
                '<=',
 ...
 */
function binarySearch(arr, target, accessor, dir = 'start', comparer = (a, b) => (a < b ? -1 : a > b ? 1 : 0)) {
    let start = 0;
...
//# sourceMappingURL=variable-aware-cache.js.map