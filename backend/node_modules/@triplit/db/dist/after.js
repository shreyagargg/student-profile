...
export function satisfiesAfter(entity, after, order) {
    const [cursor, inclusive] = after;
    if (!order || order.length !== cursor.length) {
        throw new QueryNotPreparedError('The order and after cursor are not compatible');
    }
    ...
    for (let i = 0; i < cursor.length; i++) {
        ...
        const cmp = compareValue(entityVal, cursorVal) * (orderDir === 'ASC' ? 1 : -1);
