...
let walGuard;
function initializeDatabase(payload) {
    try {
        if (db) {
            // Optional: Handle re-initialization attempts? Close old one?
            console.warn('Worker received init request but database already initialized.');
        ...
        // Prepare transaction
        transactions = {
            write: db.transaction((sets, deletes) => {
...
function startWalGuard(db, options) {
    if (walGuard) {
...
function getFullKey(key, scope) {
    return scope ? [...scope, ...key] : key;
}
function encodeKey(key, scope) {
...
addEventListener('message', (msg) => {
    ...
    if (msg.data.operation === 'init') {
        const response = initializeDatabase(request.payload);
    ...
    try {
        ...
        let iteratorId = undefined; // Specifically for scan responses
        switch (request.operation) {
            case 'get': {
                const { key, scope } = request.payload;
                const encodedKey = encodeKey(key, scope);
                const result = statements.get.get(encodedKey);
                responsePayload = result ? JSON.parse(result.value) : undefined;
                break;
            }
            case 'set': {
                const { key, value, scope } = request.payload;
                const encodedKey = encodeKey(key, scope);
            ...
            case 'scanStart': {
                const { options, scope } = request.payload;
                const low = scope
            ...
            case 'scanNext': {
                const { iteratorId: reqIteratorId } = request.payload;
            ...
            case 'applyEdits': {
                const { sets, deletes, scope } = request.payload;
                ...
                const encodedSets = sets.map(([key, value]) => [
                    encodeKey(key, scope),
        ...
        console.error(`Worker error processing operation ${request.operation} (ID: ${request.id}):`, error);
        const response = {
            id: request.id,
            type: 'error',
