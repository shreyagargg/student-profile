...
export class SqliteWorkerKvStore {
    worker;
    nextRequestId = 1;
    pendingRequests = new Map();
    activeScans = new Map(); // Key: original request ID
    isInitialized = false;
    initPromise;
    closed = false;
        ...
        this.initPromise = new Promise((resolve, reject) => {
            ...
            const initHandler = (msg) => {
                ...
                if (response.type === 'initSuccess') {
                    ...
                    this.worker.addEventListener('message', this.handleWorkerMessage.bind(this)); // Add general handler
                    resolve();
                }
                else if (response.type === 'error') {
                    console.error('Main: Worker initialization failed:', response.payload);
                    // @ts-expect-error
                    this.worker.off('message', initHandler);
                    this.worker.terminate(); // Failed init, terminate worker
                    reject(new Error(`Worker initialization failed: ${response.payload}`));
                }
            ...
            this.worker.addEventListener('error', (err) => {
                ...
                this.cleanup(
                // @ts-expect-error
                err); // Reject pending requests on worker error
    ...
    handleWorkerMessage(msg) {
        ...
        if (!pending) {
            console.warn(`Main: Received unexpected message for ID ${response.id}, Type: ${response.type}`);
            return;
        }
    ...
    handleScanMessage(response) {
        ...
        if (!activeScan || originalRequestId === undefined) {
            ...
            console.warn(`Main: Received message for unknown or completed scan iterator ${scanId}, Type: ${response.type}`);
        ...
        switch (response.type) {
            ...
            case 'scanValuesComplete':
                ...
                break;
            case 'error':
                activeScan.error = new Error(response.payload || 'Unknown scan error in worker');
    ...
    async get(key, scope) {
        return this.sendRequest('get', { key, scope });
    }
    async set(key, value, scope) {
        await this.sendRequest('set', { key, value, scope });
    }
    async delete(key, scope) {
        await this.sendRequest('delete', { key, scope });
    }
    async *scan(options, scope) {
        ...
        try {
            ...
            iteratorId = await new Promise((resolve, reject) => {
            ...
            while (true) {
                ...
                while (activeScan.buffer.length > 0) {
                    ...
                    const [key, value] = result; // Cast to expected tuple type
                    const keyWithoutPrefix = prefixLength > 0 && key.length > prefixLength
    ...
    async count(options, scope) {
        return this.sendRequest('count', { options, scope });
    }
    async clear(scope) {
        await this.sendRequest('clear', { scope });
    }
    scope(scopeTuple) {
    ...
    scope) {
        ...
        await this.sendRequest('applyEdits', {
    ...
    async close() {
        ...
        await Promise.allSettled(Array.from(this.pendingRequests.values()).map((p) => new Promise((res) => {
        ...
        try {
            await this.sendRequest('close', null); // sendRequest handles initPromise internally
        }
        catch (e) {
            console.warn('Main: Error sending close message to worker (might have already exited):', e);
        }
