...
export class BunSQLiteKVStore {
    ...
    constructor(arg0, options = {}) {
        ...
        this.transactions = {
            write: this.db.transaction((sets, deletes) => {
    ...
    startWalGuard(options) {
        ...
        if (typeof walCheck === 'object' && 'unref' in walCheck)
            walCheck.unref();
        return walCheck;
    }
    createTable() {
    ...
    get(key, scope) {
        ...
        const result = this.freeStatement(this.statements.get).get(encodedKey);
    ...
    async *scanValues(options, scope) {
        const low = scope
            ...
            : encodeTuple(options.prefix);
        const high = low + '\uffff';
        const results = this.freeStatement(this.statements.scanValues).iterate(low, high);
        for (const row of results) {
            yield JSON.parse(
            // @ts-expect-error
            row.value);
    ...
    count(options, scope) {
        ...
        if (!fullPrefix.length) {
            return Promise.resolve(this.freeStatement(this.statements.count).get()['COUNT(*)']);
        }
        const low = encodeTuple(fullPrefix);
        const high = low + '\uffff';
        return Promise.resolve(this.freeStatement(this.statements.countRange).get(low, high)['COUNT(*)']);
    }
    async clear(scope) {
