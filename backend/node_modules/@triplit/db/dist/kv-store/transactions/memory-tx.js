...
import { asyncIterConcat, asyncIterMap, asyncIterUnique, } from '../../utils/iterators.js';
import { TransactionAlreadyCanceledError, TransactionAlreadyCommittedError, } from '../../errors.js';
export class MemoryTransaction {
    storage;
    sets;
    deletes;
    _status = 'open';
    constructor(storage, sets, deletes) {
    ...
    scope(prefix) {
        return new MemoryTransaction(this.storage.scope(prefix), this.sets.scope(prefix), this.deletes.scope(prefix));
    }
     ...
     */
    async *scan(scanOptions, prefix) {
        this.checkTxStatusBeforeOperation();
        for await (const [key, value] of asyncIterUnique(asyncIterConcat(this.storage.scan(scanOptions, prefix), this.sets.scan(scanOptions, prefix), this.deletes.scan(scanOptions, prefix)), ([k]) => JSON.stringify(k))) {
    ...
    async *scanValues(scanOptions, prefix) {
        yield* asyncIterMap(this.scan(scanOptions, prefix), ([_key, value]) => value);
    }
    async count(options, prefix) {
        this.checkTxStatusBeforeOperation();
        let count = 0;
    ...
    async commit() {
        this.checkTxStatusBeforeOperation();
        await this.storage.applyEdits(this.sets.scan({ prefix: [] }), asyncIterMap(this.deletes.scan({ prefix: [] }), (it) => it[0]));
...
//# sourceMappingURL=memory-tx.js.map