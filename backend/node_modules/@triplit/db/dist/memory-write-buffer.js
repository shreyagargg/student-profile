...
export class SimpleMemoryWriteBuffer {
    _changes = {};
    constructor() { }
    clear(tx) {
        this._changes = {};
    ...
    getChangesForCollection(storage, collectionName) {
        return Promise.resolve(this._changes[collectionName]);
    }
    getChangesForEntity(storage, collectionName, id) {
...
export function mergeDBChanges(target, ...sources) {
...
export function deepIsEmpty(obj) {
    for (const prop in obj) {
        if (Object.hasOwn(obj, prop)) {
...
export function areChangesEmpty(changes) {
    return (!changes ||
        Object.values(changes).every((collectionChanges) => collectionChanges.sets.size === 0 &&
            collectionChanges.deletes.size === 0));
}
//# sourceMappingURL=memory-write-buffer.js.map