import { filterStatementIteratorFlat, isFilterGroup, isFilterStatement, isStaticFilter, isSubQueryFilter, } from '../filters.js';
...
export function extractViews(query, generateViewId) {
    const plan = {
        views: {},
        rootQuery: query,
    };
    ...
    if (query.include) {
        for (const [alias, inclusion] of Object.entries(query.include)) {
            const { newViews, rewrittenQuery } = subqueryToView(inclusion.subquery, generateViewId);
    ...
    if (query.order) {
        for (let i = 0; i < query.order.length; i++) {
            ...
            const { newViews, rewrittenQuery } = subqueryToView(maybeSubquery.subquery, generateViewId);
            Object.assign(plan.views, newViews);
...
function subqueryToView(subquery, generateViewId) {
    let newViews = null;
    let rewrittenQuery = null;
    const hasNestedSubquery = subquery.include && Object.keys(subquery.include).length > 0;
...
function whereFiltersToViews(where, generateViewId) {
    const views = {};
    const updatedWhere = [];
    for (const filter of where) {
        ...
        if (isSubQueryFilter(filter)) {
            ...
            const hasGrandparentReferences = subquery.where && hasHigherLevelReferences(subquery.where);
            const subqueryVariableFilters = (subquery.where ?? []).filter((f) => {
                return (isFilterStatement(f) &&
                    isValueVariable(f[2]) &&
            ...
            if (!hasGrandparentReferences && subqueryVariableFilters.length < 2) {
                ...
                extractedView.rootQuery.where = subquery.where?.filter((f) => !subqueryVariableFilters.includes(f));
                ...
                const vacView = VAC.queryToViews(subquery);
                const extractedView = extractViews(vacView.views[0], generateViewId);
                const { where: remainingFilters, views: remainingViews } = whereFiltersToViews(vacView.unusedFilters, generateViewId);
            ...
            const { where: newWhere, views: newViews } = whereFiltersToViews(filter.filters, generateViewId);
...
function hasHigherLevelReferences(where) {
    for (const filter of filterStatementIteratorFlat(where)) {
        if (isFilterStatement(filter) && isValueVariable(filter[2])) {
            ...
            if (hasHigherLevelReferences(filter.exists.where || []))
                return true;
        }
    }
    return false;
...
export function compileRelationalPlan(relPlan) {
    const viewSteps = {};
...
function getViewsReferencedInFilters(filters, viewNames = new Set()) {
...
export function statementHasViewReference(filter) {
 ...
 *
 */
function compileQueryToSteps(q) {
    const steps = [];
    ...
    if (q.where) {
        for (const filter of q.where) {
            if (isStaticFilter(filter)) {
                ...
                throw new TriplitError(`Could not compile query: Unsupported filter type in where clause: ${JSON.stringify(filter)}`);
            }
        }
    }
    const afterFilter = q.after
        ? {
            after: q.after,
    ...
    if (q.collectionName.startsWith('$view_')) {
        ...
        if (!all(staticFilters, isFilterStatement)) {
            throw new TriplitError(`Filters on view ${viewId} must be simple filter statements.`);
        }
        steps.push({
            type: 'PREPARE_VIEW',
            viewId,
...
export function extractInvertedViews(query) {
    const plan = {
        views: new Map(),
    ...
    if (query.include) {
        for (const [alias, inclusion] of Object.entries(query.include)) {
            const { views, rewrittenQuery } = extractInvertedViews(inclusion.subquery);
            assignViewMap(plan.views, views);
            query.include[alias] = {
                ...query.include[alias],
                subquery: rewrittenQuery,
    ...
    if (query.order) {
        for (let i = 0; i < query.order.length; i++) {
            ...
            const { views, rewrittenQuery } = extractInvertedViews(maybeSubquery.subquery);
            assignViewMap(plan.views, views);
            query.order[i][2] = { ...maybeSubquery, subquery: rewrittenQuery };
        }
    }
...
function assignViewMap(map1, map2) {
...
export function extractedInvertedViewsFromFilters(where) {
    const views = new Map();
    const updatedWhere = [];
    for (const filter of where) {
        // if the filter is a subquery filter
        ...
        if (isSubQueryFilter(filter)) {
            ...
            if (!hasGrandparentReferences && subqueryVariableFilters.length < 2) {
                ...
                // remove the variable filters from the view
                extractedView.rewrittenQuery.where = subquery.where?.filter((f) => !subqueryVariableFilters.includes(f));
            ...
            const { where: newWhere, views: newViews } = extractedInvertedViewsFromFilters(filter.filters);
            updatedWhere.push({ ...filter, filters: newWhere });
