...
import { EntityStoreWithOutbox, createDB, ValuePointer, TriplitError, queryBuilder, } from '@triplit/db';
...
export class TriplitClient {
    ...
    constructor(options = {}) {
        ...
        const dbSchema = options.schema
            ...
            : undefined;
        const storage = getClientStorage(options?.storage ?? DEFAULT_STORAGE_OPTION);
        ...
        this.awaitReady = createDB({
            ...
            this.db.onSchemaChange((change) => {
                if (change.successful) {
                    this.http.updateOptions({
                        schema: change.newSchema.collections,
                    });
                }
            });
            if (this.syncSchema) {
                this.subscribeBackground(this.db
                    .query(
                // @ts-expect-error
                '_metadata')
                    .Id('_schema'), {
            ...
            if (event.type !== 'SUCCESS') {
                ...
                if (event.type === 'SCHEMA_UPDATE_FAILED') {
                    this.logger.error('Schema update failed during initialization. The schema will fallback to the value saved in the database (change.oldSchema). For more control, set a callback in experimental.onDatabaseInit.', event);
                }
                else {
                    this.logger.error('An error occurred during database initialization', event);
                }
            }
        ...
        this.http = new HttpClient({
            ...
            token: this._token,
            schemaFactory: async () => (await this.getSchema())?.collections,
        });
        ...
        this.startSession(options.token, this.connectOnInitialization, options.refreshOptions).then(() => {
    ...
    async fetch(query, options) {
        ...
        if (opts.policy === 'local-first') {
            const isFirstTimeFetchingQuery = await this.syncEngine.isFirstTimeFetchingQuery(query);
        ...
        throw new UnrecognizedFetchPolicyError(opts.policy);
    }
    async fetchLocal(query, options) {
     ...
     */
    async clear(options = { full: false }) {
        if (this.awaitReady)
            await this.awaitReady;
        ...
        for (const sub of this.statusSubs) {
            sub.unsub();
            sub.unsub = this._subscribeWithStatus(sub.query, sub.callback, sub.options);
        }
    }
    async reset(options = {}) {
    ...
    async entityIsInCache(collection, entityId) {
        return !!this.db.entityStore.doubleBuffer.getChangesForEntity(this.db.kv, collection, entityId);
    }
        ...
        const unsubPromise = (async () => {
            ...
            onResults = async (results) => {
                ...
                if (options?.syncStatus && options.syncStatus !== 'all') {
                    ...
                    filteredResults = await this.filterResultsWithSyncStatus(results, query.collectionName, options.syncStatus);
                }
    ...
    _subscribeWithStatus(query, callback, options) {
        let results = undefined;
        // on the first time we see a subscription, check if we are connected or will connect
        let waitingOnRemoteSync = this.probablyIntendsToConnect && !options?.localOnly;
        ...
        const fetching = () => fetchingLocal || (isInitialFetch && waitingOnRemoteSync);
        function fireSignal() {
            callback({
                results,
                error,
        ...
        const unsub = this.subscribe(query, (newResults) => {
            ...
            error = undefined;
            if (fetchingRemote) {
                const hasResponded = this.syncEngine.hasServerRespondedForQuery(query);
    ...
    async filterResultsWithSyncStatus(results, collectionName, syncStatus) {
        const bufferContents = await this.db.entityStore.doubleBuffer.getChangesForCollection(this.db.kv, collectionName);
        if (bufferContents) {
    ...
    subscribeWithPagination(query, onResults, onError, options) {
        ...
        let subscriptionResultHandler = (results) => {
            onResults(results, {
                hasNextPage: false,
        ...
        returnValue.nextPage = () => {
            this.logger.warn('There is no limit set on the query, so nextPage() is a no-op');
        };
        returnValue.prevPage = () => {
            this.logger.warn('There is no limit set on the query, so prevPage() is a no-op');
        };
        ...
        if (query.limit) {
            ...
            subscriptionResultHandler = (results) => {
                ...
                const canMoveWindowBackward = !!query.after &&
                    !!firstResult &&
                    compareCursors(query.after[0], query.order.map((o) => ValuePointer.Get(firstResult, o[0].split('.')))) > -1;
                ...
                const hasPreviousPage = pagingDirection === 'reversed'
                    ? canMoveWindowForward
                    : canMoveWindowBackward;
                const hasNextPage = pagingDirection === 'forward'
                    ? canMoveWindowForward
                    : canMoveWindowBackward;
                // Remove buffered data
                results = results.slice(canMoveWindowBackward ? 1 : 0, canMoveWindowForward ? -1 : undefined);
                ...
                rangeStart = firstDataResult
                    ? query.order.map((o) => ValuePointer.Get(firstDataResult, o[0].split('.')))
                    : undefined;
                rangeEnd = lastDataResult
                    ? query.order.map((o) => ValuePointer.Get(lastDataResult, o[0].split('.')))
                    : undefined;
                ...
                if (!hasPreviousPage && !!query.after) {
                    ...
                    returnValue.unsubscribe = this.subscribe(query, subscriptionResultHandler, onError, options);
                }
                else {
                    onResults(results, {
    ...
    subscribeWithExpand(query, onResults, onError, options) {
        ...
        returnValue.loadMore = () => {
            this.logger.warn('There is no limit set on the query, so loadMore is a no-op');
        };
        if (query.limit) {
            // Add stable order to query
     ...
     */
    updateConnectionOptions(change) {
        const { token, serverUrl, tokenRefresh } = change;
        const hasTokenChange = change.hasOwnProperty('token') && token !== this.token;
        const hasServerUrlChange = change.hasOwnProperty('serverUrl') && serverUrl !== this.serverUrl;
     ...
     */
    async startSession(token, connect = true, refreshOptions) {
        ...
        if (decoded) {
            if (tokenIsExpired(decoded)) {
                if (refreshOptions?.refreshHandler) {
                    ...
                    if (!maybeToken) {
                        this.logger.warn('An expired token was passed to startSession, and the refreshHandler was unable to provide a new token. The expired session token will be used and sync issues should be handled with onSessionError().');
                    }
                    else {
                        token = maybeToken;
    ...
    isFirstTimeFetchingQuery(...args) {
        return this.syncEngine.isFirstTimeFetchingQuery(...args);
    }
    async clearPendingChangesForEntity(...args) {
        return this.syncEngine.clearPendingChangesForEntity(...args);
    }
    async clearPendingChangesAll(...args) {
        return this.syncEngine.clearPendingChangesAll(...args);
    }
    connectionOptionsChangeHandlers = [];
    onConnectionOptionsChange(callback) {
...
function addTraceIdToQuery(query) {
    return { traceId: Math.random().toString().slice(2), ...query };
}
function mapServerUrlToSyncOptions(serverUrl) {
...
function validateServerUrl(serverUrl) {
    if (serverUrl &&
        ...
        throw new TriplitError('Invalid serverUrl provided. Must start with "http://" or "https://".');
    }
}
//# sourceMappingURL=triplit-client.js.map