...
function webSocketsAreAvailable() {
...
export class WebSocketTransport {
    ...
    sendMessage(message) {
        ...
        if (this.options.messagePayloadSizeLimit &&
            bytes > this.options.messagePayloadSizeLimit) {
            const chunks = chunkMessage(serializedMessage, Math.ceil(bytes / this.options.messagePayloadSizeLimit));
    ...
    connect(params) {
        ...
        const wsUri = `${secure ? 'wss' : 'ws'}://${domain}?${wsOptions.toString()}`;
    ...
    close(reason) {
        ...
        if (this.ws.readyState === this.ws.CONNECTING) {
            return this.ws.addEventListener('open', () => {
    ...
    onConnectionChange(callback) {
        if (this.ws)
            this.ws.onconnectionchange = callback;
    }
}
...
function chunkMessage(message, numChunks) {
    let chunks = [];
...
function friendlyReadyState(conn) {
    ...
    var WebSocketProxy = new Proxy(globalThis.WebSocket, {
        construct: function (target, args) {
            ...
            function dispatchConnectionChangeEvent() {
                ...
                if (instance.onconnectionchange &&
