...
var AuthQueryContext = react.createContext(defaultAuthQueryOptions);
...
function useSession(authClient, options) {
  var _a, _b;
  const { sessionQueryOptions, sessionKey: queryKey, queryOptions } = react.useContext(AuthQueryContext);
...
function useAuthQuery({
  ...
  const { queryOptions } = react.useContext(AuthQueryContext);
  const mergedOptions = { ...queryOptions, ...options };
  return reactQuery.useQuery({
    queryKey,
    queryFn: sessionData ? () => queryFn({ fetchOptions: { throw: true } }) : queryCore.skipToken,
    ...mergedOptions
  });
}
...
function useListAccounts(authClient, options) {
  const { listAccountsKey: queryKey } = react.useContext(AuthQueryContext);
  return useAuthQuery({
    authClient,
    queryKey,
...
var useOnMutateError = () => {
  ...
  const { optimistic } = react.useContext(AuthQueryContext);
...
function useAuthMutation({
  ...
  const queryClient = reactQuery.useQueryClient();
  const context = react.useContext(AuthQueryContext);
...
function useCreateApiKey(authClient, options) {
  const { listApiKeysKey: queryKey } = react.useContext(AuthQueryContext);
  return useAuthMutation({
    queryKey,
...
function useListDeviceSessions(authClient, options) {
  const { listDeviceSessionsKey: queryKey } = react.useContext(AuthQueryContext);
  return useAuthQuery({
    authClient,
    queryKey,
...
function useDeletePasskey(authClient, options) {
  const { listPasskeysKey: queryKey } = react.useContext(AuthQueryContext);
  return useAuthMutation({
    queryKey,
...
function useUpdateUser(authClient, options) {
  const { sessionKey: queryKey } = react.useContext(AuthQueryContext);
  return useAuthMutation({
    queryKey,
...
function useListSessions(authClient, options) {
  const { listSessionsKey: queryKey } = react.useContext(AuthQueryContext);
...
function useToken(authClient, options) {
  ...
  const { tokenKey, tokenQueryOptions, queryOptions } = react.useContext(AuthQueryContext);
  ...
  const payload = react.useMemo(() => data ? decodeJwt(data.token) : null, [data]);
  react.useEffect(() => {
...
exports.AuthQueryContext = AuthQueryContext;
exports.AuthQueryProvider = AuthQueryProvider;
exports.createAuthHooks = createAuthHooks;
exports.createAuthPrefetches = createAuthPrefetches;
exports.defaultAuthQueryOptions = defaultAuthQueryOptions;
